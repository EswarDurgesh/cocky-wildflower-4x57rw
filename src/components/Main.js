...
import { Table, TableBody, TableHead, TableRow, TableCell, TableContainer, Checkbox } from '@mui/material';
...
export const Main = () => {
    ...
    const handleDeleteCell = (id) =>{
        ...
        setpage(page<Math.ceil(duplicate.length/rowsPerPage)?page:Math.ceil(duplicate.length/rowsPerPage))
        if (searchquery !== ""){
    ...
    const handleSelectAll = (event) => {
        if (event.target.checked && searchquery === ""){
            ...
            const newSelected = duplicate.slice((page-1)*rowsPerPage, page*rowsPerPage).map(item => item.id);
    ...
    const handleSearch = (event) => {
        ...
        if (event.target.value === ""){
            ...
            const duplicate = [...userdatabase];
            const tempdatabase = duplicate.filter(item => item.name.toLowerCase().includes(query.toLowerCase()) || item.email.toLowerCase().includes(query.toLowerCase()) || item.role.toLowerCase().includes(query.toLowerCase()));
    ...
    const handleDeleteSelectedCells = () => {
        let duplicate = [...userdatabase];
        duplicate = duplicate.filter((item) => {return !selected.includes(item.id)});
        setuserdatabase(duplicate);
        setpage(page<Math.ceil(duplicate.length/rowsPerPage)?page:Math.ceil(duplicate.length/rowsPerPage))
        if (searchquery !== ""){
            let duplicate = [...filterdatabase];
    ...
    const handlePaginationLength = () => {
        return filterdatabase.length>0 || searchquery !=="" ? Math.ceil(filterdatabase.length/rowsPerPage) : Math.ceil(userdatabase.length/rowsPerPage);
    }
    ...
    return (
        ...
        <TableContainer sx ={{ width: '90%', margin:'auto'}}>
            <Table padding="none">
                <TableHead>
                    <TableRow>
                        <TableCell><Checkbox checked={selected.length>0?true:false} onClick={handleSelectAll} size="small"/></TableCell>
                ...
                <TableBody>
                    {
                        filterdatabase.length>0 || searchquery !== ""? filterdatabase.slice((page-1)*rowsPerPage, page*rowsPerPage).map((item) => {
                            const isItemSelected = isSelected(item.id);
                            const isItemEditable = isEditable(item.id);
                            return (
                            <ModifiedTableRow handleItemSelection={isItemSelected} row={item} handleEditableCondition={isItemEditable} handleCellEditChange={handleCellEditChange}
                            handleCellSelection={handleCellSelection} handleDoneChanges={handleDoneChanges} handleRevertChanges={handleRevertChanges} handleToggleCellEdit={handleToggleCellEdit} handleDeleteCell={handleDeleteCell}/>
                            )
                        }) : 

                        userdatabase.slice((page-1)*rowsPerPage, page*rowsPerPage).map((item) => {
                            const isItemSelected = isSelected(item.id);
                            const isItemEditable = isEditable(item.id);
                            return (
                            <ModifiedTableRow handleItemSelection={isItemSelected} row={item} handleEditableCondition={isItemEditable} handleCellEditChange={handleCellEditChange}
        ...
        <Footer handlePageChange={handlePageNumber} handlePaginationLength={handlePaginationLength()} handleSelection={handleDeleteSelectedCells}/>
        </>
    );
}